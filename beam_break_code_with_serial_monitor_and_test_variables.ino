...
#define SENSORPIN 4
  ...
  // Pin 8: Used to test if the correct Signal was being sent.
...
int sensorState = 0,checkOutput = 0, lastState=0, lastTestoutput = 0;         // variable for reading the pushbutton status

void setup() {
  // sets the digital pin 12 as output, this is the output frpm the sensor state.
  pinMode(12, OUTPUT);  
  // sets the digital pin 8 as input, used for testing signal output.
  pinMode(TESTREAD, INPUT);   
...
void loop(){
  ...
  // read the state of the signal sent:
  checkOutput = digitalRead(TESTREAD);
  ...
  if (checkOutput && !lastTestoutput) {
    Serial.println(checkOutput);
  } 
  if (!checkOutput && lastTestoutput) {
    Serial.println(checkOutput);
  }
  lastTestoutput = checkOutput;
